...
def search_keyword_request(query, page=1):
    """ Fait une requête sur la recherche de vocabulaire d'Isidore

    :param query: La recherche textuelle
    :param page: La page à récupérer
    """
    url = "https://isidore.science/vocabulary/search" #"?q=web&output=json"
    req = requests.get(url, params={"output": "json", "q": query, "page": page})
    data = req.json()
    for resultat in data["response"]["replies"]["content"]["reply"]:
        if isinstance(resultat["isidore"]["prefLabel"], list):
            conteneur = resultat["isidore"]["prefLabel"][0]
        else:
            conteneur = resultat["isidore"]["prefLabel"]
        title = conteneur["$"]
        print(title)
    if "page" in data["response"]["replies"] and \
            "@next" in data["response"]["replies"]["page"]:
        search_keyword_request(query, page=data["response"]["replies"]["page"]["@next"])

@group.command("keyword")
@click.argument("query")
def search_keyword(query):
    """ Fait une recherche de mot-clef sur Isidore
    """
    search_keyword_request(query)
